...
import { usePayments, useProperties, useTenants, addPayment, updatePayment, deletePayment } from "../services/firestore";
import { Plus, CreditCard, Search, Filter, Edit2, Trash2, X, IndianRupee, TrendingUp, TrendingDown, Clock } from "lucide-react";
import { format, parseISO } from "date-fns";
import { useAuth } from "../context/AuthContext";
import toast from "react-hot-toast";

const DEFAULT = { tenantName: "", propertyName: "", propertyId: "", amount: "", paymentMonth: format(new Date(), "yyyy-MM"), paymentStatus: "Paid", paidDate: format(new Date(), "yyyy-MM-dd"), notes: "" };

function PaymentModal({ payment, onClose }) {
  const { user } = useAuth();
  const { data: properties } = useProperties();
  const { data: tenants } = useTenants();
  const [form, setForm] = useState(payment ? { ...DEFAULT, ...payment } : DEFAULT);
  ...
  const handleSubmit = async (e) => {
    ...
    try {
      ...
      if (form.paymentStatus === "Pending") submitForm.paidDate = "";
      if (payment?.id) { await updatePayment(payment.id, submitForm); toast.success("Payment updated!"); }
      else { await addPayment(user.uid, submitForm); toast.success("Payment recorded!"); }
      onClose();
          ...
          <div className="grid grid-cols-1 sm:grid-cols-2 gap-4">
            <div>
              ...
              <select className={`input ${errors.tenantName ? "border-red-400 focus:ring-red-400" : ""}`} value={form.tenantName} onChange={e => set("tenantName", e.target.value)}>
                ...
                {tenants.map(t => <option key={t.id}>{t.tenantName}</option>)}
              </select>
            ...
            <div>
              ...
              <select className={`input ${errors.propertyName ? "border-red-400 focus:ring-red-400" : ""}`} value={form.propertyName} onChange={e => {
                ...
                {properties.map(p => <option key={p.id}>{p.propertyName}</option>)}
              </select>
            ...
            <div>
              ...
              <input
                type="date"
                className={`input transition-opacity ${isPending ? "opacity-50 cursor-not-allowed bg-slate-100 dark:bg-slate-900" : ""} ${errors.paidDate ? "border-red-400 focus:ring-red-400" : ""}`}
                value={form.paidDate}
      ...
      <div className="max-w-7xl mx-auto space-y-5 animate-slide-up">
        ...
        <div className="grid grid-cols-3 gap-4">
          <div className="stat-card flex items-center gap-4">
            ...
            <div><p className="text-xs text-slate-500">Total Records</p><p className="font-bold text-slate-900 dark:text-white">{payments.length}</p></div>
          </div>
          <div className="stat-card flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-emerald-100 dark:bg-emerald-900/30 flex items-center justify-center">
              <TrendingUp className="w-5 h-5 text-emerald-600" />
            </div>
            <div><p className="text-xs text-slate-500">Collected</p><p className="font-bold text-emerald-600 dark:text-emerald-400">{fmt(totalCollected)}</p></div>
          </div>
          <div className="stat-card flex items-center gap-4">
            <div className="w-10 h-10 rounded-xl bg-red-100 dark:bg-red-900/30 flex items-center justify-center">
              <TrendingDown className="w-5 h-5 text-red-500" />
            </div>
            <div><p className="text-xs text-slate-500">Pending</p><p className="font-bold text-red-500">{fmt(totalPending)}</p></div>
          </div>
        </div>
        ...
        <div className="flex flex-col sm:flex-row gap-3">
          <div className="relative flex-1">
            ...
            <input className="input pl-10" placeholder="Search by tenant or property..." value={search} onChange={e => setSearch(e.target.value)} />
          </div>
          <div className="flex items-center gap-2">
            <Filter className="w-4 h-4 text-slate-400" />
            {["All","Paid","Pending"].map(f => (
              <button key={f} onClick={() => setFilter(f)}
                className={`px-4 py-2 rounded-xl text-sm font-medium transition-all ${filter === f ? "bg-brand-600 text-white" : "btn-secondary py-2"}`}>
                {f}
              </button>
            ))}
          </div>
        ...
        {loading ? (
          ...
          <div className="card p-12 text-center">
            ...
            <h4 className="font-display text-lg font-bold text-slate-500 mb-2">No payments found</h4>
              ...
              <table className="w-full text-sm">
                <thead>
                  <tr className="bg-slate-50 dark:bg-slate-800/50 text-left">
                    {["Tenant","Property","Month","Amount","Status","Date Paid",""].map(h => (
                    ...
                    <tr key={p.id} className="hover:bg-slate-50 dark:hover:bg-slate-800/30 transition-colors">
                      <td className="px-4 py-3">
                        <div className="flex items-center gap-2">
                          ...
                          <span className="font-medium text-slate-800 dark:text-slate-200">{p.tenantName || "—"}</span>
                        </div>
                      </td>
                      <td className="px-4 py-3 text-slate-500">{p.propertyName || "—"}</td>
                      ...
                      <td className="px-4 py-3">
                        <span className={p.paymentStatus === "Paid" ? "badge-paid" : "badge-pending"}>{p.paymentStatus}</span>
                      </td>
                      <td className="px-4 py-3 text-slate-500 text-sm">{p.paidDate || "—"}</td>
                      <td className="px-4 py-3">
      ...
      {deleting && (
        <div className="modal-overlay" onClick={() => setDeleting(null)}>
          <div className="card w-full max-w-sm p-6 animate-scale-in" onClick={e => e.stopPropagation()}>
            <h4 className="font-display text-xl font-bold text-center mb-2 text-slate-900 dark:text-white">Delete Payment?</h4>
            <p className="text-sm text-slate-500 text-center mb-6">This will remove the payment record for <strong>{deleting.tenantName}</strong>.</p>
